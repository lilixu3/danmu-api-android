...
### D-006：Windows 第一版支持 Windows 10/11 x64，交付免安装 + 直装双形态

状态：用户已确认（2026-08-28）。

第一版交付 x64 双形态：免安装版（`packagePortableZip`，解压即用）与直装版
（`packageExe` 安装器）。**不支持 32 位 x86**：Node.js 官方自 v19 起不再发布
win-x86 二进制，内嵌 Node 24 运行时无 32 位 Windows 可用，用户确认按 Node
官方支持平台交付。ARM64 与商店分发进入后续路线图（Node/Skiko 均有 win-arm64
官方产物，可低成本加入构建矩阵）。
...
2. 已确认（2026-08-28）：仅 x64，32 位 x86 排除（Node 官方自 v19 起无 32 位
   Windows 二进制），ARM64 留后续——见 D-006。
3. 关闭窗口默认退出，还是默认最小化到托盘。
4. 是否需要“作为 Windows Service 运行”，还是登录用户托盘后台进程即可。
5. 已确认（2026-08-28）：免安装（ZIP）与直装（EXE）双形态第一版都要——见 D-006。
