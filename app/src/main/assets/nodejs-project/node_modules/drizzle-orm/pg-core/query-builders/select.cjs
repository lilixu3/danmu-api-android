...
var PgSelectBase = class extends __query_builders_query_builder_ts.TypedQueryBuilder {
	...
	*/
	_resolveSelection() {
		...
		config.fieldsFlat ??= (0, __utils_ts.orderSelectedFields)(config.fields, void 0, dialect.codecs);
		const { fieldsFlat, setOperators } = config;
		if (setOperators.length && !config.setFieldsFlat) {
			const setSelection = Array.from({ length: fieldsFlat.length });
			for (let i = 0; i < setOperators.length; ++i) {
				...
				for (let j = 0; j < fieldsFlat.length; ++j) {
					...
					setSelection[j].codecOverride = lc && rc ? require_pg_core_codecs.unionsTypeTable[lc]?.[rc] : lc;
				}
			}
