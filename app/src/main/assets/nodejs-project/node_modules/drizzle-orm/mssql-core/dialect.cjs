	...
	*/
	buildSelection(fields, { isSingleTable = false, table } = {}) {
		...
		for (let i = 0; i < columnsLen; ++i) {
			...
			if ((0, __entity_ts.is)(field, __column_ts.Column)) if (isSingleTable) chunks.push(field.isAlias ? __sql_sql_ts.sql`${__sql_sql_ts.sql.identifier((0, __alias_ts.getOriginalColumnFromAlias)(field).name)} as ${field}` : __sql_sql_ts.sql.identifier(field.name));
				...
				if (!isSingleTable && field.origin !== void 0) chunks.push(__sql_sql_ts.sql.identifier(field.origin), __sql_sql_ts.sql.raw("."));
			...
			else if ((0, __entity_ts.is)(field, __subquery_ts.Subquery)) if (!field._.isWith) chunks.push(__sql_sql_ts.sql`(${field._.sql}) ${__sql_sql_ts.sql.identifier(field._.alias)}`);
			else chunks.push(field);
	...
	buildSelectionOutput(fields, { type }) {
		...
		for (let i = 0; i < columnsLen; ++i) {
			const { field } = fields[i];
			if ((0, __entity_ts.is)(field, __column_ts.Column)) chunks.push(__sql_sql_ts.sql.join([__sql_sql_ts.sql.raw(`${type}.`), field.isAlias ? __sql_sql_ts.sql`${__sql_sql_ts.sql.identifier((0, __alias_ts.getOriginalColumnFromAlias)(field).name)} as ${field}` : __sql_sql_ts.sql.identifier(field.name)]));
			else if ((0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased)) if (field.isSelectionField) chunks.push(__sql_sql_ts.sql.join([__sql_sql_ts.sql.raw(`${type}.`), __sql_sql_ts.sql.identifier(field.fieldAlias)]));
			else {
				const query = field.sql;
				chunks.push(new __sql_sql_ts.SQL(query.queryChunks.map((c) => {
