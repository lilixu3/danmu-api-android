	...
	*/
	buildSelection(fields, { isSingleTable = false, table } = {}) {
		...
		for (let i = 0; i < columnsLen; ++i) {
			...
			if (is(field, Column)) if (isSingleTable) chunks.push(field.isAlias ? sql`${sql.identifier(getOriginalColumnFromAlias(field).name)} as ${field}` : sql.identifier(field.name));
				...
				if (!isSingleTable && field.origin !== void 0) chunks.push(sql.identifier(field.origin), sql.raw("."));
			...
			else if (is(field, Subquery)) if (!field._.isWith) chunks.push(sql`(${field._.sql}) ${sql.identifier(field._.alias)}`);
			else chunks.push(field);
	...
	buildSelectionOutput(fields, { type }) {
		...
		for (let i = 0; i < columnsLen; ++i) {
			const { field } = fields[i];
			if (is(field, Column)) chunks.push(sql.join([sql.raw(`${type}.`), field.isAlias ? sql`${sql.identifier(getOriginalColumnFromAlias(field).name)} as ${field}` : sql.identifier(field.name)]));
			else if (is(field, SQL.Aliased)) if (field.isSelectionField) chunks.push(sql.join([sql.raw(`${type}.`), sql.identifier(field.fieldAlias)]));
			else {
				const query = field.sql;
				chunks.push(new SQL(query.queryChunks.map((c) => {
