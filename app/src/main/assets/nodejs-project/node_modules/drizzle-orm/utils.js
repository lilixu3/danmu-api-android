		...
		for (const [pathChunkIndex, pathChunk] of path.entries()) if (pathChunkIndex < path.length - 1) {
			...
			if (joinsNotNullableMap && path.length >= 2) {
				...
				if (isColumn) {
					...
					else if (typeof nullifyMap[objectName] === "string" && nullifyMap[objectName] !== tableName) nullifyMap[objectName] = false;
				}
			}
		}
		return result;
	}, {});
	if (joinsNotNullableMap && Object.keys(nullifyMap).length > 0) for (const [objectName, tableName] of Object.entries(nullifyMap)) {
...
function makeJitQueryMapperInner(columns, joinsNotNullableMap) {
	...
	for (let idx = 0; idx < columns.length; ++idx) {
		...
		const objectName = path[0];
		if (allFieldIds[objectName] === void 0) allFieldIds[objectName] = [`c${idx}`];
	...
	for (let idx = 0; idx < columns.length; ++idx) {
		...
		for (let d = commonLen; d < objectPath.length; ++d) fn.push(`${"	".repeat(d + 1)}${jsonPath[d]}: ${d === 0 && joinsNotNullableMap && typeof objectName === "string" && !joinsNotNullableMap[objectName] ? `${allFieldIds[pathPrefix].map((c) => `${c} === null`).join(" && ")} ? null : {` : "{"}`);
...
function makeDefaultQueryMapper(columns, joinsNotNullableMap) {
	const interpretedData = Array.from({ length: columns.length });
	for (let i = 0; i < columns.length; ++i) {
		...
		const objectName = joinsNotNullableMap && path.length >= 2 ? path[0] : void 0;
		const tableName = objectName !== void 0 && isColumn ? getTableName(field.table) : void 0;
		interpretedData[i] = [
			decoder,
			objectName,
			tableName
	...
	return ((rows) => {
		...
		const output = Array.from({ length: rowLength });
