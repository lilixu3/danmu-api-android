...
- 支持一键切换、更新、回退
- 支持安装自定义核心
- 稳定版、开发版和自定义版更新时统一检测运行时依赖
- 稳定版只查 stable 签名依赖包，开发版只查 dev 签名依赖包，禁止跨通道复用
- 自定义核心不查询依赖仓库；缺依赖时取消更新并保留旧核心
...
- **签名依赖包**：核心升级时按依赖指纹安全补齐纯 JavaScript 依赖，详见 [核心运行时依赖包](docs/runtime-dependency-packs.md)
