      ...
      >
        <section class="panel">
          <div class="panel-heading">
            <div>
              <h2>基础内容</h2>
              <p class="muted">先把公告本身写清楚，App 会优先展示标题、简介和正文摘要。</p>
            </div>
          </div>
        ...
        <section class="panel">
          <div class="panel-heading">
            <div>
              <h2>展示方式</h2>
              <p class="muted">这里决定这条公告属于短期还是长期，以及在 App 里用什么方式提醒用户。</p>
            </div>
          </div>
        ...
        <section class="panel">
          <div class="panel-heading">
            <div>
              <h2>发布时间</h2>
              <p class="muted">控制什么时候开始展示，以及这次保存后要处于什么状态。</p>
            </div>
          </div>
          ...
          <div class="form-grid">
            ...
            <div class="field field-callout">
              <span>App 上的关闭方式</span>
              <div class="callout-box" id="dismiss_behavior_hint"></div>
            </div>
        ...
        <section class="panel">
          <div class="panel-heading">
            <div>
              <h2>跳转按钮</h2>
              <p class="muted">每个按钮都可以选择跳到 App 指定页面，或者跳转到你自定义的网页链接。</p>
            </div>
          </div>

          <div class="form-grid">
            <div class="field field-full action-config-card">
              <span>主按钮</span>
...
${secondaryRouteOptions}
        ...
        <section class="panel">
          <div class="panel-heading">
            <div>
              <h2>内容预览</h2>
              <p class="muted">正文修改后可刷新预览，确认摘要是否清晰。</p>
            </div>
            <button type="button" id="preview-trigger" class="btn btn-ghost">刷新预览</button>
          </div>
    ...
    <script>
      ...
      const announcementTypeHint = document.getElementById('announcement_type_hint');
      const dismissBehaviorHint = document.getElementById('dismiss_behavior_hint');
      const endAtInput = document.getElementById('end_at');
      const endAtHint = document.getElementById('end_at_hint');
      const primaryButtonMode = document.getElementById('primary_button_mode');
      const secondaryButtonMode = document.getElementById('secondary_button_mode');
      const primaryButtonRouteField = document.getElementById('primary_button_route_field');
      const primaryButtonUrlField = document.getElementById('primary_button_url_field');
      const secondaryButtonRouteField = document.getElementById('secondary_button_route_field');
      const secondaryButtonUrlField = document.getElementById('secondary_button_url_field');
      ...
      async function refreshPreview() {
        try {
          ...
          if (response.redirected || response.status === 401 || response.status === 403 || !contentType.includes('application/json')) {
          ...
          previewText.textContent = '预览暂时不可用，请稍后重试。';
        }
      }

      function syncAnnouncementTypeHint() {
        const type = announcementTypeSelect.value;
        if (type === 'short') {
          announcementTypeHint.textContent = '短期公告适合活动、限时维护、短期通知。必须填写结束时间。';
          dismissBehaviorHint.textContent = '短期公告在 App 中不会显示“今日不提醒”，关闭按钮会按“不再提示”处理。';
          endAtHint.textContent = '短期公告必须填写结束时间，过期后会自动不再展示。';
          endAtInput.required = true;
        } else {
          announcementTypeHint.textContent = '长期公告适合常驻说明、持续提醒、长期活动入口。';
          dismissBehaviorHint.textContent = '长期公告在普通弹窗下会显示“今日不提醒”，方便用户当天不再重复看到。';
          endAtHint.textContent = '长期公告可不填结束时间，留空表示长期有效。';
          endAtInput.required = false;
        }
      }

      function syncActionFields(modeSelector, routeField, urlField) {
        const mode = modeSelector.value;
        routeField.style.display = mode === 'app_route' ? '' : 'none';
        urlField.style.display = mode === 'link' ? '' : 'none';
      }

      previewTrigger.addEventListener('click', refreshPreview);
      announcementTypeSelect.addEventListener('change', syncAnnouncementTypeHint);
      primaryButtonMode.addEventListener('change', () => {
        syncActionFields(primaryButtonMode, primaryButtonRouteField, primaryButtonUrlField);
      });
      secondaryButtonMode.addEventListener('change', () => {
      ...
      textarea.addEventListener('input', () => {
        ...
        window.__previewTimer = setTimeout(refreshPreview, 300);
      });
